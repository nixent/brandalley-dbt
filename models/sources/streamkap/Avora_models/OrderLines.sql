SELECT sha1(CONCAT(sfoi_con.product_id,sfoi_con.order_id,sfoi_con.item_id,sfo.entity_id,IFNULL (cpev_pt_con.value,'_'),IFNULL (eaov_brand.option_id,'_'),IFNULL (eaov_color.option_id,'_'),IFNULL (eaov_size.option_id,'_'),IFNULL (cpei_size_child.entity_id,'_'),IFNULL (eaov_size_child.option_id,'_'))) AS unique_id,
       ...
       CASE
         WHEN sfoi_con.dispatch_date = '0000-00-00' THEN NULL
       ...
       CAST((sfoi_sim.base_cost) AS DECIMAL (20,6)) AS product_cost_exc_vat,
       ...
       CAST((((sfoi_con.original_price*sfoi_sim.qty_ordered) /(1 +(sfoi_con.tax_percent / 100.))) -(sfoi_sim.base_cost*sfoi_sim.qty_ordered)) /((sfoi_con.original_price*sfoi_sim.qty_ordered) /(1 +(sfoi_con.tax_percent / 100.))) AS DECIMAL (20,6)) AS margin_inc_discount_percentage,
       CAST((((sfoi_con.original_price*sfoi_sim.qty_ordered) /(1 +(sfoi_con.tax_percent / 100.))) -(sfoi_sim.base_cost*sfoi_sim.qty_ordered)) AS DECIMAL (20,6)) AS margin_inc_discount_value,
       CAST(((((sfoi_con.original_price*sfoi_sim.qty_ordered) - sfoi_con.discount_amount) /(1 +(sfoi_con.tax_percent / 100.))) -(sfoi_sim.base_cost*sfoi_sim.qty_ordered)) /((sfoi_con.original_price*sfoi_sim.qty_ordered) /(1 +(sfoi_con.tax_percent / 100.))) AS DECIMAL (20,6)) AS margin_exc_discount_percentage,
       CAST((((sfoi_con.original_price*sfoi_sim.qty_ordered) - sfoi_con.discount_amount) /(1 +(sfoi_con.tax_percent / 100.))) -(sfoi_sim.base_cost*sfoi_sim.qty_ordered) AS DECIMAL (20,6)) AS margin_exc_discount_value,
...
FROM streamkap.sales_flat_order sfo
  LEFT JOIN streamkap.sales_flat_order_item sfoi_sim
         ON sfoi_sim.order_id = sfo.entity_id
        AND sfoi_sim.product_type = 'simple'
  LEFT JOIN streamkap.sales_flat_order_item sfoi_con
  ...
  LEFT JOIN streamkap.catalog_product_entity_varchar cpev_outletcat_con
  ...
  LEFT JOIN streamkap.catalog_product_entity_varchar cpev_outletcat_sim
  ...
  LEFT JOIN streamkap.catalog_product_entity_varchar cpev_pt_sim
         ON cpev_pt_sim.entity_id = sfoi_sim.product_id
        AND cpev_pt_sim.attribute_id = 179
        AND cpev_pt_sim.store_id = 0
  LEFT JOIN streamkap.eav_attribute_option_value eaov_pt_sim
         ON cpev_pt_sim.value = eaov_pt_sim.option_id
        AND eaov_pt_sim.store_id = 0
  LEFT JOIN streamkap.catalog_product_entity_varchar cpev_pt_con
         ON cpev_pt_con.entity_id = sfoi_con.product_id
        AND cpev_pt_con.attribute_id = 179
        AND cpev_pt_con.store_id = 0
  LEFT JOIN streamkap.eav_attribute_option_value eaov_pt_con
         ON cpev_pt_con.value = eaov_pt_con.option_id
        AND eaov_pt_con.store_id = 0
  LEFT JOIN streamkap.catalog_product_entity_int cpei_brand
         ON cpei_brand.entity_id = sfoi_con.product_id
        AND cpei_brand.attribute_id = 178
        AND cpei_brand.store_id = 0
  LEFT JOIN streamkap.eav_attribute_option_value eaov_brand
         ON eaov_brand.option_id = cpei_brand.value
        AND eaov_brand.store_id = 0
  LEFT JOIN streamkap.catalog_product_entity_int cpei_color
         ON cpei_color.entity_id = sfoi_con.product_id
        AND cpei_color.attribute_id = 213
        AND cpei_color.store_id = 0
  LEFT JOIN streamkap.eav_attribute_option_value eaov_color
         ON eaov_color.option_id = cpei_color.value
        AND eaov_color.store_id = 0
  LEFT JOIN streamkap.catalog_product_entity_int cpei_size
         ON cpei_size.entity_id = sfoi_con.product_id
        AND cpei_size.attribute_id = 177
        AND cpei_size.store_id = 0
  LEFT JOIN streamkap.eav_attribute_option_value eaov_size
         ON eaov_size.option_id = cpei_size.value
        AND eaov_size.store_id = 0
  LEFT JOIN streamkap.catalog_product_entity_int cpei_size_child
         ON cpei_size_child.entity_id = sfoi_sim.product_id
        AND cpei_size_child.attribute_id = 177
        AND cpei_size_child.store_id = 0
  LEFT JOIN streamkap.eav_attribute_option_value eaov_size_child
         ON eaov_size_child.option_id = cpei_size_child.value
        AND eaov_size_child.store_id = 0
  LEFT JOIN streamkap.catalog_product_entity_int cpei_supplier
         ON cpei_supplier.entity_id = sfoi_sim.product_id
        AND cpei_supplier.attribute_id = 239
        AND cpei_supplier.store_id = 0
  LEFT JOIN streamkap.catalog_product_supplier cps_supplier ON cpei_supplier.value = cps_supplier.supplier_id
  LEFT JOIN streamkap.sales_flat_order_item_extra sfoie ON sfoi_con.item_id = sfoie.order_item_id
  LEFT JOIN streamkap.catalog_category_entity_history cceh ON sfoie.category_id = cceh.category_id
  LEFT JOIN streamkap.catalog_product_negotiation cpn ON sfoi_sim.nego = cpn.negotiation_id
  LEFT JOIN streamkap.admin_user au ON cpn.buyer = au.user_id
  LEFT JOIN streamkap.catalog_product_super_link cpsl ON sfoi_sim.product_id = cpsl.product_id
  LEFT OUTER JOIN streamkap.catalog_product_entity cpe ON cpe.entity_id = cpsl.parent_id
  LEFT JOIN streamkap.catalog_product_entity_decimal cped_price
         ON sfoi_sim.product_id = cped_price.entity_id
        AND cped_price.attribute_id = 75
        AND cped_price.store_id = 0
  LEFT JOIN streamkap.catalog_product_entity cpe_ref ON sfoi_sim.sku = cpe_ref.sku
  LEFT JOIN streamkap.catalog_product_reference cpr ON cpe_ref.entity_id = cpr.entity_id
WHERE sfo.increment_id NOT LIKE '%-%'
AND   (sfo.sales_product_type != 12 OR sfo.sales_product_type IS NULL)
GROUP BY sha1(CONCAT(sfoi_con.product_id,sfoi_con.order_id,sfoi_con.item_id,sfo.entity_id,IFNULL (cpev_pt_con.value,'_'),IFNULL (eaov_brand.option_id,'_'),IFNULL (eaov_color.option_id,'_'),IFNULL (eaov_size.option_id,'_'),IFNULL (cpei_size_child.entity_id,'_'),IFNULL (eaov_size_child.option_id,'_'))),
         sfo.increment_id,
         sfo.customer_id,
         sfoi_sim.item_id,
         sfo.entity_id,
...
,
         ...
         CAST((sfoi_sim.base_cost) AS DECIMAL (20,6)),
         (sfoi_sim.base_cost)*sfoi_sim.qty_ordered,
         ...
         CAST((((sfoi_con.original_price*sfoi_sim.qty_ordered) /(1 +(sfoi_con.tax_percent / 100.))) -(sfoi_sim.base_cost*sfoi_sim.qty_ordered)) /((sfoi_con.original_price*sfoi_sim.qty_ordered) /(1 +(sfoi_con.tax_percent / 100.))) AS DECIMAL (20,6)),
         CAST((((sfoi_con.original_price*sfoi_sim.qty_ordered) /(1 +(sfoi_con.tax_percent / 100.))) -(sfoi_sim.base_cost*sfoi_sim.qty_ordered)) AS DECIMAL (20,6)),
         CAST(((((sfoi_con.original_price*sfoi_sim.qty_ordered) - sfoi_con.discount_amount) /(1 +(sfoi_con.tax_percent / 100.))) -(sfoi_sim.base_cost*sfoi_sim.qty_ordered)) /((sfoi_con.original_price*sfoi_sim.qty_ordered) /(1 +(sfoi_con.tax_percent / 100.))) AS DECIMAL (20,6)),
         CAST((((sfoi_con.original_price*sfoi_sim.qty_ordered) - sfoi_con.discount_amount) /(1 +(sfoi_con.tax_percent / 100.))) -(sfoi_sim.base_cost*sfoi_sim.qty_ordered) AS DECIMAL (20,6)),
