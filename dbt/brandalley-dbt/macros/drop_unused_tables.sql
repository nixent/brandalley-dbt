...
with last_queries as (
  ...
  rt.dataset_id
FROM
  region-europe-west2.INFORMATION_SCHEMA.JOBS, unnest(referenced_tables) as rt
...
left join last_queries lq on t.table_schema = lq.dataset_id and t.table_name = lq.table_id
