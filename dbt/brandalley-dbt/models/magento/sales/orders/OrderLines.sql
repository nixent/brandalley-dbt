...
with order_lines as (
	select
		...
		sum(sfoi_sim.qty_ordered * (sfoi_con.base_price_incl_tax /nullif((1 + (sfoi_con.tax_percent / 100.)),0)) - sfoi_con.base_discount_amount) 			as TOTAL_GBP_ex_tax_after_vouchers,
		sum(sfoi_sim.qty_ordered * (sfoi_con.base_price_incl_tax /nullif((1 + (sfoi_con.tax_percent / 100.)),0))) 											as TOTAL_GBP_ex_tax_before_vouchers,
