...
with order_lines as (
	select
		...
		SAFE_DIVIDE(sfoi_con.discount_amount, 1 + (sfoi_con.tax_percent / 100.))																	        as line_discount_amount_exc_vat,
		...
		case
			...
			when lower(ccfse.path_name) like '%>clearance>%' and date(created_at) not in ('2023-12-22', '2023-12-23', '2023-12-24') then 'Clearance'
