...
case when wait_time < 10 then '0-10 sec'
     when wait_time < 20 then '10-20 sec'
     when wait_time < 30 then '20-30 sec'
     when wait_time < 60 then '30-60 sec'
     when wait_time < 300 then '60-300 sec'
     else '>300 sec' end                                                                                                as wait_time_bucket,
case when duration < 300 then '0-5 min'
     when wait_time < 600 then '5-10 min'
     when wait_time < 900 then '10-15 min'
     when wait_time < 1200 then '15-20 min'
