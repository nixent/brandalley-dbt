...
select
    ...
    category_id,
    name,
    timestamp(sale_start) as sale_start,
    timestamp(sale_end) as sale_end,
    type,
    nego,
    delivery_info,
...
from {{ ref('stg_uk__catalog_category_entity_history') }}
{% if is_incremental() %}
...
select
    ...
    __deleted,
    bq_last_processed_at
from {{ ref('stg_fr__catalog_category_entity_history') }}
{% if is_incremental() %}
