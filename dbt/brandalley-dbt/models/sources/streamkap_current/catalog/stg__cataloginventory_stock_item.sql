...
select
    ...
    is_in_stock,
    timestamp(low_stock_date) as low_stock_date,
    notify_stock_qty,
...
from {{ ref('stg_uk__cataloginventory_stock_item') }}
{% if is_incremental() %}
...
select
    ...
    __deleted,
    bq_last_processed_at
from {{ ref('stg_fr__cataloginventory_stock_item') }}
{% if is_incremental() %}
