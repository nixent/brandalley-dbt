...
with order_stats as (
    select
        date_trunc(if(ba_site = 'FR',datetime(created_at, "Europe/Paris"),datetime(created_at, "Europe/London")), week(monday))                  as order_created_at_week,
        ba_site,
...
customer_stats as (
    select
        date_trunc(if(ba_site = 'FR',datetime(signed_up_at, "Europe/Paris"),datetime(signed_up_at, "Europe/London")), week(monday))           as customer_created_at_week,
        ba_site,
...
refund_stats as (
    select
        date_trunc(if(sfc.ba_site = 'FR',datetime(timestamp(sfc.created_at), "Europe/Paris"),datetime(timestamp(sfc.created_at), "Europe/London")), week(monday))       as order_created_at_week,
        sfc.ba_site,
...
shipping_stats as (
    select
        date_trunc(if(ba_site = 'FR',datetime(timestamp(order_date), "Europe/Paris"),datetime(timestamp(order_date), "Europe/London")), week(monday)) as order_created_at_week,
        ba_site,
...
order_line_stats as (
    select
        date_trunc(if(o.ba_site = 'FR',datetime(o.created_at, "Europe/Paris"),datetime(o.created_at, "Europe/London")), week(monday))                                  as order_created_at_week,
        ol.ba_site,
...
cs_stats as (
    select
        date_trunc(date, week(monday)) as cs_tickets_week,
...
margin_adjustments as (
    select 
        date_trunc(date, week(monday)) as margin_week,
...
select
    os.order_created_at_week || '-' || os.ba_site as ba_site_created_date,
...
left join refund_stats rs
    on os.order_created_at_week = rs.order_created_at_week and os.ba_site = rs.ba_site
left join shipping_stats ss
    on os.order_created_at_week = ss.order_created_at_week and os.ba_site = ss.ba_site
left join order_line_stats ols
    on os.order_created_at_week = ols.order_created_at_week and os.ba_site = ols.ba_site
left join conversion_stats cs
    on os.order_created_at_week = datetime(cs.ga_session_at_week) and os.ba_site = 'UK'
left join customer_stats cs2
    on os.order_created_at_week = cs2.customer_created_at_week and os.ba_site = cs2.ba_site
left join cs_stats css
    on os.order_created_at_week = css.cs_tickets_week and os.ba_site = 'UK'
left join margin_adjustments ma
    on os.order_created_at_week = ma.margin_week and os.ba_site = 'UK'
